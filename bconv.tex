...
\begin{proof} Note that $K$ must be linearly bounded, so $\|\cdot\|_K$ is a norm and it is easy to verify that $S=co(K\cup-K)$ is the closed unit ball. Let $\{x_n\}$ be a Cauchy sequence, then it is norm-bounded, so we may assume that $\{x_n\}\subset S$. Note that $S$ is $\tau$-compact as well. Let $y\in S$ be a $\tau$-accumulation point of $\{x_n\}$. For $\epsilon>0$, $\|x_n-x_m\|_K<\epsilon$ for $n,m\ge N$. This implies that
...
this holds since any subspace $E\subseteq X$ satisfies $E^{\perp\perp}=E$. However, this is no longer true for subspaces in $X'$ (\cite{kothe}), so a dual statement does not hold. More precisely, it is easily checked that $P\cap -P\subseteq (P'-P')^\perp$, so that 
 ...
 (P\cap -P)^\perp$, so $P'$ may be not generating even of $P$ is a cone (there are indeed counterexamples).
\end{rem}
...
\begin{lemma} 
\begin{enumerate}
\item[(i)] Any $\varphi\in P'$ is $\|\cdot\|_u$-bounded, with $\|\varphi\|_u^*:=\sup_{\|x\|_u\le 1}|\varphi(x)|=\varphi(u)$.
\item[(ii)] If $\varphi\in X'$ is such that $\|\varphi\|_u^*=\varphi(u)$, then $\varphi\in P'$.
\item[(iii)] For $x\in X$, $\|x\|_u=\sup_{\varphi\in \mathcal S(X,P,u)} |\varphi(x)|$.
\end{enumerate}
